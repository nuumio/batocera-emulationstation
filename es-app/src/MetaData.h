...
public:
	...
	void addTag(const std::string& value) { auto r = mTags.insert(value); if (r.second) setDirty(); };
	void removeTag(const std::string& value) { if(mTags.erase(value) > 0) setDirty(); };
